...
import mod.minisse.elementals.entity.NuibariEntity;
...
public class NuibariItem extends Item implements IVanishable {
   private final Multimap<Attribute, AttributeModifier> tridentAttributes;

   public NuibariItem(Properties builderIn) {
      super(builderIn);
         ...
         if (i >= 10) {
            ...
            if (j <= 0 || playerentity.isWet()) {
               if (!worldIn.isRemote) {
                  ...
                  if (j == 0) {
                     NuibariEntity nuibarientity = new NuibariEntity(worldIn, playerentity, stack);
                     nuibarientity.setDirectionAndMovement(playerentity, playerentity.rotationPitch, playerentity.rotationYaw, 0.0F, 2.5F + (float)j * 0.5F, 1.0F);
                     if (playerentity.abilities.isCreativeMode) {
                        nuibarientity.pickupStatus = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
                     }

                     worldIn.addEntity(nuibarientity);
                     worldIn.playMovingSound((PlayerEntity)null, nuibarientity, SoundEvents.ITEM_TRIDENT_THROW, SoundCategory.PLAYERS, 1.0F, 1.0F);
                     if (!playerentity.abilities.isCreativeMode) {
