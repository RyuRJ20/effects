...
public class TestMonsterEntity extends MonsterEntity {
   ...
   private static final DataParameter<Boolean> IS_CHILD = EntityDataManager.createKey(TestMonsterEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> VILLAGER_TYPE = EntityDataManager.createKey(TestMonsterEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Boolean> DROWNING = EntityDataManager.createKey(TestMonsterEntity.class, DataSerializers.BOOLEAN);
   ...
   public TestMonsterEntity(EntityType<? extends TestMonsterEntity> type, World worldIn) {
      super(type, worldIn);
   }

   public TestMonsterEntity(World worldIn) {
      this(EntityType.ZOMBIE, worldIn);
   }

   protected void registerGoals() {
      this.goalSelector.addGoal(4, new TestMonsterEntity.AttackTurtleEggGoal(this, 1.0D, 3));
   ...
   protected void func_234341_c_(EntityType<? extends TestMonsterEntity> p_234341_1_) {
      TestMonsterEntity zombieentity = this.func_233656_b_(p_234341_1_, true);
   ...
   public boolean attackEntityFrom(DamageSource source, float amount) {
      if (!super.attackEntityFrom(source, amount)) {
         ...
         if (event.getResult() == net.minecraftforge.eventbus.api.Event.Result.ALLOW  ||
            ...
            TestMonsterEntity zombieentity = event.getCustomSummonedAid() != null && event.getResult() == net.minecraftforge.eventbus.api.Event.Result.ALLOW ? event.getCustomSummonedAid() : EntityType.ZOMBIE.create(this.world);
   ...
   public void onKillEntity(ServerWorld world, LivingEntity killedEntity) {
      ...
      if ((world.getDifficulty() == Difficulty.NORMAL || world.getDifficulty() == Difficulty.HARD) && killedEntity instanceof VillagerEntity && net.minecraftforge.event.ForgeEventFactory.canLivingConvert(killedEntity, EntityType.ZOMBIE_VILLAGER, (timer) -> {})) {
         ...
         zombievillagerentity.onInitialSpawn(world, world.getDifficultyForLocation(zombievillagerentity.getPosition()), SpawnReason.CONVERSION, new TestMonsterEntity.GroupData(false, true), (CompoundNBT)null);
   ...
   public ILivingEntityData onInitialSpawn(IServerWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {
      ...
      if (spawnDataIn == null) {
         spawnDataIn = new TestMonsterEntity.GroupData(func_241399_a_(worldIn.getRandom()), true);
      }

      if (spawnDataIn instanceof TestMonsterEntity.GroupData) {
         TestMonsterEntity.GroupData zombieentity$groupdata = (TestMonsterEntity.GroupData)spawnDataIn;
   ...
   class AttackTurtleEggGoal extends BreakBlockGoal {
      ...
      public void playBreakingSound(IWorld worldIn, BlockPos pos) {
         worldIn.playSound((PlayerEntity)null, pos, SoundEvents.ENTITY_ZOMBIE_DESTROY_EGG, SoundCategory.HOSTILE, 0.5F, 0.9F + TestMonsterEntity.this.rand.nextFloat() * 0.2F);
      }
